...
class VoxelnpzPngSingleDeblurDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        event_frames = [f for f in event_frames if not os.path.basename(f).startswith("._")]  # ✅ Ignore macOS hidden files
        ...
        # 
        if len(sharp_frames) == 0:
            print("⚠️ No sharp images detected. Running test without sharp ground truth.")
            self.dataPath = [
                {'blur_path': blur, 'sharp_path': None, 'event_paths': event}
                for blur, event in zip(blur_frames, event_frames)
            ]
        else:
            assert len(blur_frames) == len(sharp_frames) == len(event_frames), \
                f"Mismatch in blur ({len(blur_frames)}), sharp ({len(sharp_frames)}), and event ({len(event_frames)}) frame counts."
            self.dataPath = [
                {'blur_path': blur, 'sharp_path': sharp, 'event_paths': event}
                for blur, sharp, event in zip(blur_frames, sharp_frames, event_frames)
            ]

        # till now

        for i in range(len(blur_frames)):
            sharp_path = sharp_frames[i] if i < len(sharp_frames) else None  # ✅ Define sharp_path properly
            self.dataPath.append({
                'blur_path': blur_frames[i],
                'sharp_path': sharp_path,  # ✅ Now correctly defined
    ...
    def __getitem__(self, index):
        ...
        # YGP 3.0
        if gt_path is None:
            # Ensure img_lq is a tensor before using zeros_like
            if not isinstance(img_lq, torch.Tensor):
                img_lq = torch.tensor(img_lq)  # Convert NumPy array to PyTorch tensor
            
            img_gt = torch.zeros_like(img_lq)  # ✅ Now works correctly
            print(f"⚠️ No sharp image for {image_path}. Using dummy tensor.")
        else:
            img_bytes = self.file_client.get(gt_path)    # 'gt'
            img_gt = imfrombytes(img_bytes, float32=True)
        # till here

        #  YGP voxel = np.load(event_path)['voxel']

        voxel = np.load(event_path, allow_pickle=True)['voxel']
        ## Data augmentation
        # voxel shape: h,w,c
        # crop
